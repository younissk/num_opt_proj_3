...
class ForwardBackward:
    ...
    def __init__(self, problem: SineApproximationProblem, lambda_param: float):
        """
    ...
    def solve(self, 
        ...
        for k in range(max_iterations):
            # Store current state
            current_objective = self.problem.objective_function(x) + self.lambda_param * np.sum(np.abs(x))
            current_prox_residual = self.problem.compute_proximal_residual(x, self.lambda_param)
            ...
            if current_prox_residual < tolerance:
                if verbose:
                    print(f"Converged at iteration {k}: proximal residual = {current_prox_residual:.2e}")
                break
            ...
            x_new = self.problem.l1_proximal_operator(x_temp, self.lambda_param * self.step_size)
    ...
    def get_optimality_conditions(self, x: np.ndarray) -> Dict[str, Any]:
        ...
        grad_f = self.problem.gradient(x)
        x_temp = x - self.step_size * grad_f
        prox_result = self.problem.l1_proximal_operator(x_temp, self.lambda_param * self.step_size)
